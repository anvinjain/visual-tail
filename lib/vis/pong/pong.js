...
Pong.prototype.start = function () {
    console.log("Pong visualization started");
    var self = this;

    var renderer = Phaser.AUTO;

    if(this.bogey.params && this.bogey.params.renderer) {
...
Pong.prototype._addRequest = function (request) {
    ...
    leftSections.forEach(function (section) {
        if (section.ip && section.ip.alive && section.ip.ip === request.clientIp) {
            leftSection = section;
    ...
    rightSections.forEach(function (section) {
        if (section.request && section.request.clientIp === request.clientIp) {
    ...
    var ball = new Ball(this.game, request, 0, leftSection.y, {
        ...
        miss: request.status.toString().slice(0, 1) === '4',
        error: request.status.toString().slice(0, 1) === '5'
    });
    ...
    if (!leftSection.ip || leftSection.ip.ip !== request.clientIp) {
        // Add the IP address.
        var ip = new Ip(this.game, 0, leftSection.y, request.clientIp, ball.colorIndex);
        this.ipGroup.add(ip);
...
Pong.prototype._updateBalls = function (request) {
    ...
    self.ballGroup.forEach(function (ball) {
        ...
        var statusCode = ball.request.status.toString();
...
Pong.prototype._handleCollision = function (ball, paddle) {
    ...
    if (ball.request.status.toString().slice(0, 1) === '5') {
    ...
    var status = new Status(this.game, this.paddle.x - 15, ball.y, ball.request.status.toString());
...
Pong.prototype._addUri = function (ball, paddle) {
    ...
    // Create the URI.
    var uri = new Uri(this.game, paddle.x + 10, ball.dest.y, ball.request.uri, ball.colorIndex);
